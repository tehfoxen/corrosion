<?
class Model_Quicksearch extends Model
{
	...
	public function SearchResult(){
		...
		//ищем в гостах: ищем поисковую фразу в табл. gost, затем смотрим, какие методы в этот гост входят и какие сущности привязаны к этим методам
		$stmt = DB::run("SELECT t1.name as value, t3.id as entity_id FROM `gost`t1
							LEFT JOIN `method` t2 ON (t2.gost_id = t1.id)
							LEFT JOIN `entity` t3 ON (t3.method_id = t2.id)
							WHERE t1.name LIKE $search_str");
		...
		//ищем в методах табл. method
		$stmt = DB::run("SELECT t1.name as value,t2.id as entity_id FROM `method`t1
